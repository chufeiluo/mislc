...
def gen_questions_query(gen_text, sentenece_tokens, mask, tokenizer):
    ...
    for span in spans:
        ...
        if args.query_model_source != 'openai':
            sampling_params= SamplingParams(temperature=0.3, top_p=1, max_tokens=64, logprobs=1)
            outputs = query_gen_pipeline.generate(
                        prompt,
                        sampling_params=sampling_params,
                                        use_tqdm=False,
                                )
...
def flare_sentence_gen(prompt, model, tokenizer, config, device, retriever, theta, beta, max_new_tokens):
    # temporary sentence generation
    generated_tokens, probs, generated_text, finish_reason = one_sentence_generation(prompt.format(retrieved_doc=""), model, tokenizer, device, max_new_tokens)
    ...
    if is_retrieval_triggered(probs, theta):
        ...
        queries.append(masked_sentence_query(generated_tokens, mask, tokenizer))
        ...
        if retrieved_doc is not None:  # in case nothing matches
            ...
            generated_tokens, ـ, _, _ = one_sentence_generation(prompt.format(retrieved_doc=retrieved_doc), model, tokenizer, device, max_new_tokens)
...
def flare_generation(prompt, model, tokenizer, config, device, retriever):
    generation_str = ""
    generation_length = 0
    queries = []
    while True:
        gen_tokens, new_queries = flare_sentence_gen(prompt + generation_str, model, tokenizer, config, device, retriever, args.theta, args.beta, args.max_new_tokens)
...
def generate_outputs(df, model, tokenizer, config, device, retriever, prompt_template, outp_fn='output.csv'):
    ...
    df.to_csv(os.path.join(args.output_dir, outp_fn), index=False)
...
def main(args):
    ...
    generate_outputs(dataset_df, model, tokenizer, config, device, searcher, prompt_template, args.output_name)
